...
## 8. UX 규칙 — 결과 화면 Enter 프롬프트 일관 적용

### 원칙

데이터를 표시한 후 즉시 반환되는 모든 화면에
`[Enter] 메뉴로 돌아가기` 프롬프트를 추가한다.
이 규칙이 없으면 `MainView`의 `clearScreen()`이 결과를 즉시 지운다.

### 적용 대상 전체

| View | 위치 | 유형 |
|------|------|------|
| `OrderView.placeOrder()` | 오류 / 취소 / 접수 완료 | 완료·오류 |
| `ApprovalView.run()` | 승인 대기 없음 | 빈 목록 |
| `ApprovalView.processApproval()` | 시료 오류 | 오류 |
| `ApprovalView.handleApprovalConfirm()` | 승인 완료 (CONFIRMED/PRODUCING) / 거절 완료 | 완료·결과 |
| `ProductionView.run()` | 생산 대기 없음 (IDLE) | 빈 목록 |
| `ProductionView.completeProduction()` | 취소 / 생산 완료 | 완료·취소 |
| `ReleaseView.run()` | 출고 가능 주문 없음 | 빈 목록 |
| `ReleaseView.processRelease()` | 취소 / 출고 완료 | 완료·취소 |
| `SampleView.listSamples()` | 빈 목록 / 목록 끝 | 조회 |
| `SampleView.searchSample()` | 결과 없음 / 결과 끝 | 조회 |
| `MonitoringView.showOrderSummary()` | 목록 끝 | 조회 |
| `MonitoringView.showStockStatus()` | 목록 끝 | 조회 |

---

## 9. 버그 수정 — OrderView Enter 안내 없음
