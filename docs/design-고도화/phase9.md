...
`Paginator.printNavBar()` 및 각 View의 페이지 입력 프롬프트가 120자 너비에서 좌측에 치우쳐 있다.

| 대상 | 변경 |
|------|------|
| `Paginator.printNavBar()` | 앞 공백 2자 → 40자로 중앙 정렬 |
| `SampleView` `[P/N]` 프롬프트 | 앞 공백 추가 (약 45자) |
| `ApprovalView` / `ReleaseView` 페이지 프롬프트 | 앞 공백 추가 (약 37자) |
| 번호 선택 프롬프트 (`승인할 번호 >` 등) | 앞 공백 추가 (약 53자) |

### 8-2. 헤더 행 테두리 불일치

한글 헤더의 display 너비가 Java 문자열 길이보다 커서 `│` 위치가 밀렸다.

| View | 현상 | 수정 |
|------|------|------|
| `ApprovalView` | trailing 25자 → 헤더 125 display | trailing 20자로 수정 (120 display) |
| `ReleaseView` | trailing 7자 → 헤더·데이터 121 display | trailing 6자로 수정 (120 display) |

### 8-3. ApprovalView `[0] 위로` 테이블 내부 오류

`[0] 위로`가 테이블 행 안에 포함되어 있었다. 입력 프롬프트에 이미 포함되어 있으므로 테이블에서 제거한다.

**함께 개선:** 주문번호 컬럼(22) + 시료명을 별도 행으로 표시하던 2줄 구조를
단일 행으로 통합한다. 컬럼 구성:
- 번호(5) + 주문번호(26, 확장) + 시료명(36) + 고객명(36) + 수량(8) + trailing(2)
- 검증: 2+5+26+1+36+1+36+1+8+2 = 117 → `│  ...  │` = 1+117+1 = **119**... 아니면 trailing(3): 1+(2+5+26+1+36+1+36+1+8+3)+1 = **121**

> **구현 시 trailing을 정확히 계산해 120자로 맞춘다.**
