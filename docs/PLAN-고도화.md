...
## 5. Phase 8 — 뒤로가기 입력 통일

**목표:** 모든 화면에서 상위 메뉴로 돌아가는 방법을 일관되게 통일한다.

**통일 원칙:**

| 화면 유형 | 현재 | 변경 후 |
|-----------|------|---------|
| 조회 전용 화면 (목록, 재고 현황 등) | `Enter` | `[0] 위로` |
| 선택 가능 목록 (승인, 출고 등) | `[0]` | `[0]` (유지) |
| 액션 완료·오류·취소 화면 | `Enter` | `Enter` (유지) |
| 빈 목록 안내 화면 | `Enter` | `Enter` (유지) |
| 서브메뉴 루프 | `[0]` | `[0]` (유지) |

**변경 대상:** 조회 전용 화면만 `Enter` → `[0]` 으로 변경한다.

| 위치 | 메서드 |
|------|--------|
| `MonitoringView` | `showOrderSummary()` |
| `MonitoringView` | `showStockStatus()` |
| `SampleView` | `listSamples()` 비페이지네이션 경로 |
| `SampleView` | `searchSample()` 비페이지네이션 경로 |

**완료 기준:**
- [ ] 조회 화면에서 `[0]` 으로 이전 메뉴 복귀 확인
- [ ] 완료·오류 화면은 `Enter` 유지 확인
- [ ] 기존 테스트 전체 통과

---

## 6. Phase 9 — CMD 가로 너비 확장

**목표:** 현재 64자 기준의 화면 너비를 120자로 확장해 정보를 더 넓게 표시한다.

**현재 → 변경:**

| 요소 | 현재 | 변경 후 |
|------|------|---------|
| `SEPARATOR` | `=`.repeat(64) | `=`.repeat(120) |
| `THIN_LINE` | `-`.repeat(64) | `-`.repeat(120) |
| `TABLE_TOP/DIVIDER/BOTTOM` (기본) | 내부 62 | 내부 118 |
| `ReleaseView` 표 | 내부 80 | 내부 118 (통일) |
| 배너·부제 위치 | 좌측 | 120자 기준 중앙 정렬 |
| 각 `printf` 컬럼 너비 | 기존 | 늘어난 여백 배분 |

**주요 영향 파일:** `ConsoleHelper.java`, `MainView.java`, `SampleView.java`,  
`ApprovalView.java`, `ReleaseView.java`, `MonitoringView.java`, `ProductionView.java`

**완료 기준:**
- [ ] 모든 표 테두리가 120자 너비에서 정상 출력 확인
- [ ] 배너 중앙 정렬 확인
- [ ] 기존 테스트 전체 통과

---

## 7. Phase 10 — 추가 화면 표 테두리

**목표:** Phase 7에서 누락된 표 형식 화면에 박스 드로잉 테두리를 적용한다.

**대상:**

| 화면 | 메서드 | 비고 |
|------|--------|------|
| `MonitoringView` | `showStockStatus()` | 재고량 확인 — 시료별 현황 표 |
| `MonitoringView` | `showOrderSummary()` | 주문량 확인 — 상태별 현황 표 |
| `ProductionView` | `printWaitingQueue()` | 생산라인 대기 큐 — FIFO 순서 표 |
